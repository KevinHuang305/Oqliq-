...
const fs = require('fs');
class DummyElement {
    constructor(id) { this.id = id; this.style = {}; this.classList = { toggle: ()=>{}, add: ()=>{}, remove: ()=>{} }; }
    addEventListener() {}
    getContext() { return { clearRect: ()=>{}, drawImage: ()=>{}, beginPath: ()=>{}, arc: ()=>{}, fill: ()=>{}, stroke: ()=>{}, moveTo: ()=>{}, lineTo: ()=>{}, fillText: ()=>{}, strokeRect: ()=>{}, fillRect: ()=>{}, measureText: ()=>({width: 0}), setLineDash: ()=>{} }; }
}
global.document = {
    getElementById: (id) => new DummyElement(id),
    querySelectorAll: () => [],
};
global.window = {
    addEventListener: () => {},
...
global.alert = console.log;
        ...
        window.switchCanvasView = function (view) {
            currentView = view;
            document.getElementById('btnViewFront').className = view === 'front' ? 'btn btn-primary' : 'btn btn-ghost';
            document.getElementById('btnViewSide').className = view === 'side' ? 'btn btn-primary' : 'btn btn-ghost';
            document.getElementById('btnAiDetect').style.display = view === 'front' ? 'block' : 'none';
        ...
        window.selectMeasurePart = function (labelName, inputId) {
            ...
            let otherLine = views[otherView].measureLines.find(l => l.baseLabel === labelName);
            
            if (otherLine && (labelName === '胸圍' || labelName === '腰圍' || labelName === '臀圍')) {
                ...
                finalVal = perimeter.toFixed(1);
                finalLabel = `${labelName}(精準)`;
                isPrecise = true;
                
                otherLine.name = finalLabel;
                otherLine.val = finalVal;
            }

            if (inputId) {
                document.getElementById(inputId).value = finalVal;
                if (isPrecise) {
                    document.getElementById(inputId).style.color = "#10b981";
        ...
        document.getElementById('imageUpload').addEventListener('change', function (e) {
            ...
            reader.onload = e => { views.front.img.src = e.target.result; };
        ...
        document.getElementById('imageUploadSide').addEventListener('change', function (e) {
            ...
            cStatus.innerText = "圖片載入中...";
            const reader = new FileReader();
            reader.onload = e => { views.side.img.src = e.target.result; };
            reader.readAsDataURL(e.target.files[0]);
        });
        ...
        const downEvent = d => {
            if (!views[currentView].img.src) return;
            const p = getCoords(d);
        ...
        const moveEvent = m => {
            ...
            if (draggingPoint) {
                ...
                if (draggingPoint.line === views[currentView].refLine) views[currentView].pxPerCm = dist(views[currentView].refLine.p1, views[currentView].refLine.p2) / views[currentView].refLine.real;
                redraw(); return;
            }
        ...
        function redraw() {
            ...
            views[currentView].measureLines.forEach(l => {
                const multi = l.multiplier || 1;
                const reVal = (dist(l.p1, l.p2) / views[currentView].pxPerCm * multi).toFixed(1);
                l.val = reVal;
                if (l.inputId) {
                    document.getElementById(l.inputId).value = reVal;
                }
                l.labelBox = drawLine(l.p1, l.p2, '#60a5fa', `${l.name}: ${l.val}`);
            });
        ...
        function drawLine(p1, p2, c, txt, isTop = false) {
            ...
            const fontSize = Math.max(16 * scale, 14);
            ...
            if (txt) {
                ...
                if (isTop) {
                    ...
                    let tempW = ctx.measureText(txt).width;
                ...
                let rx = x - w / 2 - 10 * scale, ry = y - fontSize - 10 * scale, rw = w + 20 * scale, rh = fontSize + 16 * scale;
        ...
        document.getElementById('btnAiDetect').onclick = function () {
            ...
            const realH = parseFloat(heightInput);
            document.getElementById('refHeightInput').value = realH;

            cStatus.innerText = "AI 運算中...";
            setTimeout(() => {
                const res = poseLandmarker.detect(views[currentView].img);
                if (res.landmarks && res.landmarks.length > 0) {
                    ...
                    views[currentView].refLine = { p1: { x: tX(nose.x), y: tY(headY) }, p2: { x: tX((pose[27].x + pose[28].x) / 2), y: tY(footY) }, real: realH };
                    views[currentView].pxPerCm = dist(views[currentView].refLine.p1, views[currentView].refLine.p2) / realH;
                    ...
                    const [c_L, c_R] = makeLine(sL, sR, 0.95, 40 * canvas.width / 800);
                    const [w_L, w_R] = makeLine(hL, hR, 1.3, -120 * canvas.width / 800);
                    const [hp_L, hp_R] = makeLine(hL, hR, 1.7, -35 * canvas.width / 800);

                    const shV = calc(e_sL, e_sR); views[currentView].measureLines.push({ p1: e_sL, p2: e_sR, name: '肩寬', val: shV, multiplier: 1, inputId: 'val-shoulder' }); document.getElementById('val-shoulder').value = shV;
                    const chV = calc(c_L, c_R, 2.6); views[currentView].measureLines.push({ p1: c_L, p2: c_R, name: '胸圍(推估)', val: chV, multiplier: 2.6, inputId: 'val-chest' }); document.getElementById('val-chest').value = chV;
                    const waV = calc(w_L, w_R, 2.4); views[currentView].measureLines.push({ p1: w_L, p2: w_R, name: '腰圍(推估)', val: waV, multiplier: 2.4, inputId: 'val-waist' }); document.getElementById('val-waist').value = waV;
                    const hipV = calc(hp_L, hp_R, 2.2); views[currentView].measureLines.push({ p1: hp_L, p2: hp_R, name: '臀圍(推估)', val: hipV, multiplier: 2.2, inputId: 'val-hip' }); document.getElementById('val-hip').value = hipV;
        ...
        function submitFinalData(bagNo) {
            ...
            if (views[currentView].img.src && uploadedFileName !== "無照片") {
                formData.append("fileData", canvas.toDataURL("image/jpeg", 0.8));
            }
        ...
        window.measureNext = function () {
            ...
            views[currentView].img.removeAttribute('src');
            views[currentView].img.src = '';

            window.resetCanvas();
            setTimeout(() => { ctx.clearRect(0, 0, canvas.width, canvas.height); }, 10);

            uploadedFileName = "無照片";
        ...
        window.showModal = function (r) {
            ...
            const views[currentView].imgEl = document.getElementById('modalImg'), loadText = document.getElementById('loadingImg');
            views[currentView].imgEl.style.display = 'none'; loadText.style.display = 'block';
            if (r['照片連結'] && r['照片連結'].startsWith('http')) { loadText.innerText = "載入照片中..."; views[currentView].imgEl.src = getDriveThumbnailUrl(r['照片連結']); } else { loadText.innerText = "無照片紀錄"; }
        };

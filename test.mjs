        ...
        let APPS_SCRIPT_URL = localStorage.getItem('api_url') || DEFAULT_API_URL;
        let SYSTEM_BASE_URL = localStorage.getItem('base_url') || (window.location.origin + window.location.pathname);
        ...
        function renderAdminBoard() {
            if (document.getElementById('view-admin').classList.contains('active')) {
                ...
                document.getElementById('dashPending').innerText = allRecords.filter(r => !r['狀態'] || r['狀態'] === '待確認').length;
                document.getElementById('dashPrep').innerText = allRecords.filter(r => r['狀態'] === '備料中').length;
                document.getElementById('dashBagged').innerText = allRecords.filter(r => r['狀態'] === '已裝袋').length;
                document.getElementById('dashCollected').innerText = allRecords.filter(r => r['狀態'] === '已領取').length;
                ...
                tr.innerHTML = `
                    ...
                    <td onclick="event.stopPropagation();">
                        <select class="quick-status-sel st-${st}" onchange="quickUpdateStatus(this, '${r['裝袋序號']}')" style="padding: 6px; border-radius: 4px; font-size: 13px; font-weight: 700; border-width: 1px; border-style: solid; min-width: 90px; outline: none; -webkit-appearance: none; appearance: none; text-align: center;">
                            ...
                            <option value="已領取" ${st==='已領取'?'selected':''}>已領取</option>
        ...
        window.renderLabels = function (type) {
            ...
            const copies = (type === 'squad' || type === 'unit') ? (parseInt(document.getElementById('globalCopiesInp').value) || 1) : 1;
            ...
            if (type === 'inner') {
                ...
                Object.keys(squads).forEach((k, idx) => {
                    ...
                    for(let i=0; i<copies; i++) {
                        const qrId = `qr_squad_${idx}_${i}`;
                        html += `<div class="label-card">${getPrintTool()}<div class="label-text"><div style="font-weight:bold; font-size:16px; border-bottom:2px solid #000; margin-bottom:10px; display:flex; align-items:center;"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:6px;"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"></path><polyline points="3.27 6.96 12 12.01 20.73 6.96"></polyline><line x1="12" y1="22.08" x2="12" y2="12"></line></svg>SQUAD BOX</div>
                        ...
                        labelQRCodes.push({ id: qrId, text: `${SYSTEM_BASE_URL}?scan=squad&box=${encodeURIComponent(boxNo)}` });
                    }
                });
            }
            else if (type === 'unit') {
                ...
                Object.keys(units).forEach((u, idx) => {
                    for(let i=0; i<copies; i++) {
                        ...
                        html += `<div class="label-card">${getPrintTool()}<div class="label-text"><div style="font-weight:bold; font-size:16px; border-bottom:2px solid #000; margin-bottom:10px; display:flex; align-items:center;"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:6px;"><rect x="4" y="2" width="16" height="20" rx="2" ry="2"></rect><path d="M9 22v-4h6v4"></path><path d="M8 6h.01"></path><path d="M16 6h.01"></path><path d="M12 6h.01"></path><path d="M12 10h.01"></path><path d="M12 14h.01"></path><path d="M16 10h.01"></path><path d="M16 14h.01"></path><path d="M8 10h.01"></path><path d="M8 14h.01"></path></svg>UNIT SUPPLY CRATE</div>
            ...
            setTimeout(() => { labelQRCodes.forEach(qr => { try { new QRCode(document.getElementById(qr.id), { text: qr.text, width: 96, height: 96 }); } catch (e) { } }); }, 100);
        };
        ...
        if (scanMode) {
            ...
            if (!DEFAULT_ADMIN_PWD) {
                const p = prompt("掃碼功能需管理員權限，請輸入密碼：");
                if (p) {
                    localStorage.setItem('admin_pwd', p);
                    DEFAULT_ADMIN_PWD = p;
                    window.location.reload();
                } else {
                    document.getElementById('scanSubtitle').innerText = "未登入，無法執行掃碼";
                }
            } else {
        ...
        window.saveSettings = function () {
            ...
            localStorage.setItem('base_url', baseUrl || (window.location.origin + window.location.pathname));
            localStorage.setItem('api_url', apiUrl || DEFAULT_API_URL);
        ...
        window.renderScanResult = function (scanMode, params) {
            ...
            if (scanMode === 'person') {
                ...
                if (records.length > 0) {
                    ...
                    document.getElementById('scanSubtitle').innerText = "分隊箱掃描";
                    area.innerHTML = `
                        <div style="font-size:20px; font-weight:bold; margin-bottom:10px; color:var(--primary);">箱號：${box}</div>
        ...
        window.executeScanAction = async function (newStatus) {
            ...
            if (!confirm(`確定要將這 ${currentScanRecords.length} 筆資料的狀態變更為「${newStatus}」？`)) return;

            const btns = document.querySelectorAll('#scanActionArea button');
            btns.forEach(b => b.disabled = true);

            let successCount = 0;
            document.getElementById('scanSubtitle').innerText = `處理中... (0/${currentScanRecords.length})`;

            for (let i = 0; i < currentScanRecords.length; i++) {
                const record = currentScanRecords[i];
                const formData = new URLSearchParams();
                formData.append("action", "updateRecord");
                formData.append("bagNo", record['裝袋序號']);
                formData.append("status", newStatus);
                try {
                    const res = await fetch(APPS_SCRIPT_URL, { method: "POST", body: formData });
                    const result = await res.json();
                ...
                document.getElementById('scanSubtitle').innerText = `處理中... (${i + 1}/${currentScanRecords.length})`;
            }
